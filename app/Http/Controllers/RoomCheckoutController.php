...
use App\{RoomCategory, User, Wallet, Setting, TempCheckout, UsedPromocode, Promocode, Book, Transaction, RefundUsers};
...
class RoomCheckoutController extends Controller
{
  ...
  public function saveRoomTempData(Request $request)
  {
    ...
    if (!empty($checkout_form_data)) {
      ...
      $phone = '+91'. $checkout_form_data['customerPhone'];
      $id = User::where('phone_number', $phone)->first();
      $data  = [];

      $data['txn_type'] = 'ROOM';
      $data['user_id'] = (!Auth::check() && isset($id)) ? $id->id : Auth::user()->id;
  ...
  public function testMAil(){
    $email="rakesh.j@mailinator.com";
    $txn_rec = Transaction::first();
    $setting = Setting::whereIn('key',['phone_number','address'])->get();
    dispatch(new RoomConfirmJob($email,$txn_rec,$setting));
  }
}
