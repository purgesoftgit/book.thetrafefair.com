...
use App\{RoomCategory, Setting, TempCheckout, UsedPromocode, Promocode, Book, Transaction, Wallet, PhoneVerification, User, NotificationPreferece, UserAddress, RefundUsers};
...
class RoomCheckoutController extends Controller
{
    public function roomcart($order_id)
    {
        ...
        if (empty($temp_data)) {
            ...
            return view('room-checkout.checkout', compact('address', 'credit_amount', 'reward_amount', 'promocodes', 'form_data', 'order_id', 'per_room_person', 'no_of_rooms', 'roomcategory', 'per_room_childrens_allowed'));
        }
    }
    ...
    public function saveRoomTempData(Request $request)
    {
        ...
        if (!empty($checkout_form_data)) {
            ...
            $data['meal_amount'] = $sum;

            if ($request->food_items)
                $data['food_items'] = $checkout_form_data['food_items'];
            ...
            $category = ($checkout_form_data['room_category'] == 'super_deluxe') ? 'deluxe' : $checkout_form_data['room_category']; //because of get same amount for deluxe or super deluxe

            $charges = Setting::where('key', $category . '_room_charges')->first();

            //calculate total amount
            $data['totalAmount'] = ($sum * $checkout_form_data['guest']) + ($checkout_form_data['diffDays'] * ($available_price * $checkout_form_data['room']));
    ...
    {
        $data = $request->all();
        $user = User::where('phone_number', '+91' . $data['phone'])->orWhere('email', $data['email'])->count();
        ...
        if ($user == 0) {
            ...
            $this->registerUser($userdata, $randomString);
        }
        ...
        $guest = $data['guest'];
        $randomstr = (isset($data['randomstr']) && !empty($data['randomstr'])) ?  $data['randomstr'] : '';
        $phone_number = $data['phone'];


        if ($randomstr != '') {
            $is_phone_valid = PhoneVerification::where('phone', '+91' . $phone_number)->where('randomstring', $randomstr)->count();
            if ($is_phone_valid < 1) {
                return response()->json(['status' => 'incorrect-phone']);
            } else {
                // PhoneVerification::where('phone','+91'.$phone_number)->where('randomstring',$randomstr)->delete();
            }
        }


        $room = RoomCategory::with('roomadditionaldata')->where('room_category', $room_category_filter)->first();
    ...
    public function room_payment_success($txn_id)
    {
        if ($txn_id != '') {
            ...
            $checkout_form_data = !empty($transaction_data) ? json_decode($transaction_data['checkout_form_data'], true) : array();
            $email = $checkout_form_data['customerEmail'];
    ...
    public function deductAmountForDeluxe(Request $request){
        ...
        if($room_Category){
            ...
            return response()->json(['final_amount'=>number_format((float)$total_amount
            , 2, '.', ''),'room_tax'=>(int)$room_tax,'meal_amount'=>$subtotal_meal_amt,'food_tax'=>(int)$food_tax,'tax_include_taxes'=>number_format((float)$tax_include_taxes, 2, '.', ''),'promo_amount'=>$promo_amount,'payable_amount'=>$payable_amount]);die;
        }
      }
      ...
      public function applyPromoCode(Request $request){
        ...
        if($roomcategory == "restaurant"){
          ...
          if($promo_id != 0 && in_array($promo_id,$arr)){
            echo json_encode(array('status'=>'error', 'message'=>'Invalid promocode')); die;
          }else if(round($datediff) > 1 && $roomcategory == "deluxe"){
            echo json_encode(array('status'=>'error', 'message'=>'You can Book Room only for 1 Night with this promocode.')); die;
          }else if($no_of_rooms > 1 && $roomcategory == "deluxe"){
            echo json_encode(array('status'=>'error', 'message'=>'You can Book only 1 Room with this promocode.')); die;
          } else{
        ...
        if(($checkin >= $validity_start_date && $checkin <= $validity_to_date) || ($checkout >= $validity_start_date && $checkout <= $validity_to_date)){
            if((time() >= $validity_start_time && time() <= $validity_to_time) || (time() >= $validity_start_time && time() <= $validity_to_time)){
              if($net_total_amt  < $min_order_value){
                  echo json_encode(array('status'=>'error', 'message'=>'Minimum order must be '.$min_order_value.' Rs.')); die;
                }else{
                  if($type_of_customer == 'new'){
                    ...
                    if(empty($is_txn_exist)){
                      ...
                      echo json_encode(array('status'=>'success', 'message'=>'Congrats!! Promocode has applied.','dicounted_amt'=>$dicounted_amt,'net_amount_data'=>$net_amount_data)); die;
                    }else{
                      echo json_encode(array('status'=>'error', 'message'=>'Offer is applicable for first time user only.')); die;
                    }
                  }else{
                    if($type_of_discount == 'net'){
                      $dicounted_amt = $discount;
              ...
              echo json_encode(array('status'=>'error', 'message'=>'Offer is not valid at this time.')); die;
            }
        }else{
            echo json_encode(array('status'=>'error', 'message'=>'Offer is not valid at this time.')); die;
        }
      ...
      public function updateRoomPrices($promodata,$subtotal_meal_amt,$order_id,$additional_amount){
        ...
        return array('final_amount'=>number_format((float)$promodata->max_discount
        , 2, '.', ''),'room_tax'=>(float)$room_tax,'meal_amount'=>$subtotal_meal_amt,'food_tax'=>(int)$food_tax,'tax_include_taxes'=>number_format((float)$tax_include_taxes, 2, '.', ''),'promo_amount'=>$promo_amount,'payable_amount'=>$payable_amount);
                        ...
                        <table border="0" cellpadding="0" cellspacing="0" style="width:100%;">
                          <tr>
                            ...
                            <td valign="top" width="100">
                              <img src="' . env('APP_URL') . 'img/brand-img.png" alt="Img" style="text-align: right; width:100px;">
                            </td>
                          </tr>
                        </table>
                      </td>
                      ...
                      <td>
                        <table border="0" cellpadding="0" cellspacing="0" style="width:100%;">
                          ...
                          <tr>
                            ...
                            <th style="font-size:12px; font-weight:700; color:#000; text-align: left; padding:0 10px 4px;">Hotel The Trade International</th>
                          ...
                          <tr>
                            ...
                            <td style="font-size:12px; color:#666; width:280px; padding:0 10px 4px;">
                    ...
                    <tr>
                      <td>
                        <table border="0" cellpadding="0" cellspacing="0" height="100%" width="100%" style="background: #fffcf1; padding: 12px 12px 3px;">
                            ...
                            <tr>
                              <td valign="top" style="width:70%;">
                                <div style="font-size: 12px; color: #222; font-weight: 700; font-family: "Raleway", sans-serif; margin:0;">Meal Total Amount</div>
                              </td>
                              <td valign="top" style="width:30%; text-align: right; font-size:12px;">Rs.' . $checkout_form_data['subtotal_meal_amt'] . '</td>
                            </tr>
                            <tr>
                              <td height="2"></td>
                              <td height="2"></td>
                            ...
                            <tr>
                              <td valign="top" style="width:70%;">
                                <div style="font-size: 12px; color: #222; font-weight: 700; font-family: "Raleway", sans-serif; margin:0;">Meal Tax</div>
                              </td>
                              <td valign="top" style="width:30%; text-align: right; font-size:12px;">Rs.' . $checkout_form_data['subtotal_meal_tax'] . '</td>
                            </tr> 
                            <tr>
                              <td height="2"></td>
                              <td height="2"></td>
                            ...
                            <tr>
                                <td valign="top" style="width:70%;">
                                  <div style="font-size: 12px; color: #222; font-weight: 700; font-family: "Raleway", sans-serif; margin:0;">Trade International Credit</div>
                                </td>
                                <td valign="top" style="width:30%; text-align: right; font-size:12px;">- ' . $checkout_form_data['subtotal_tti_credit'] . ' P</td>
                            </tr> 
                            <tr>
                                <td height="2"></td>
                                <td height="2"></td>
                            ...
                            <tr>
                                <td valign="top" style="width:70%;">
                                  <div style="font-size: 12px; color: #222; font-weight: 700; font-family: "Raleway", sans-serif; margin:0;">Reward Point</div>
                                </td>
                                <td valign="top" style="width:30%; text-align: right; font-size:12px;">- ' . $checkout_form_data['subtotal_tti_rewardpoint'] . ' P</td>
                            </tr>
                             <tr>
