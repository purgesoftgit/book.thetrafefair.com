  ...
  function getRoom(is_first){
    ...
    if((localStorage.getItem("index-filter") != null && localStorage.getItem("index-filter") == "true" || localStorage.getItem("index-filter") == true) && localStorage.getItem("check-in") != null && localStorage.getItem("check-out") != null && localStorage.getItem("room-guest") != null && localStorage.getItem("category")){
    ...
    $.ajax({
        ...
        'type': 'POST',
        'data': {'checkin':$('#datepicker').val(),'checkout':$('#checkout').val(),'room_category_filter':$('#room_category_filter').val(),'room_guest':$('.rooms_guests p').text(), "limit":limit , "_token": "{{ csrf_token() }}",'is_first':is_first},
  ...
  $(document).ready(function(){
    ...
    $(document).on('click','.search-room',function(){
      ...
      getAvailsRooms($('.check-in').val(),$('#room_category_filter option:selected').attr("data-id"));
  ...
  $('.check-out,.check-in').on('change',function(){
      ...
      if(startdate < moment().format('YYYY-MM-DD') || startdate > moment().add(2, 'months').format('YYYY-MM-DD')){
        $('.check-in-error').show()
        $('.check-in-error').text("Start Date cannot be before Today Date.").css({'display':'block','font-size':'13px','color':'red'}).delay( 800 ).fadeOut( 1000 );
      ...
      if(enddate < moment().format('YYYY-MM-DD') || enddate > moment().add(2, 'months').add(1, 'days').format('YYYY-MM-DD')){
        $('.check-out-error').text("End Date cannot be before Today Date.").css({'display':'block','font-size':'13px','color':'red'}).delay( 800 ).fadeOut( 1000 );
        $('.search-room').prop("disabled",true)
      }
 ...
 //room abd guest start
  $(document).on('click','.rooms_guests',function(e){ e.stopPropagation(); $('.rooms_guests_list').toggle(); });
  $(document).on('click','.rooms_guests_list',function(event){ event.stopPropagation(); });
  $(document).on('click','.added_rooms .editDiv .room-guest-edit-remove',function(event){ event.stopPropagation(); });
  ...
  $('.room_number_child ul li').click(function(){
    ...
    ($(this).text() == 0) && $('.child-1-block, .child-2-block, .child-3-block, .child-4-block').hide();
    ($(this).text() == 1) && $('.child-1-block').show(), $('.child-2-block,.child-3-block,.child-4-block').hide();
    ($(this).text() == 2) && $('.child-1-block, .child-2-block').show(), $('.child-3-block,.child-4-block').hide();
    ($(this).text() == 3) && $('.child-1-block, .child-2-block,.child-3-block').show(),$('.child-4-block').hide();
    ($(this).text() == 4) && $('.child-1-block, .child-2-block,.child-3-block,.child-4-block').show();
  });

 $(document).change('#child_oneage,#child_twoage,#child_threeage,#child_fourage',function(){ $('.child_age_error').hide();})
  ...
  $('.apply-changes').click(function(){
        ...
        $('.added_rooms').children().each(function(key,value){
            count_rooms = key+1;
            count_adults = parseInt($(value).find("p").text().split(', ')[0].trim().split('Adults')[0])
            count_child = 0;
  ...
  var is_child = true;
   $('.add-other-room').click(function(){
        ...
        html += '<div class="roomRow'+i+'"><strong>Room '+i+'</strong><div class="editDiv"><p>'+adults+' Adults</p><div class="room-guest-edit-remove"><a class="edit-rooms" onclick="editRooms('+i+')" style="text-decoration:none;font-size: 14px;font-weight: 600;cursor:pointer;">Edit</a><a class="removeButton" onclick="removeRow('+i+')" style="font-size: 14px;font-weight: 600;">Remove</a></div></div><div class="editDetail" style="display:none;"><p>ADULTS (12y +)</p><div class="new_guest_list"><div class="row room_number room_number_adults"><ul><li onclick="editRoomAdults(1,'+i+')">1</li><li onclick="editRoomAdults(2,'+i+')">2</li><li onclick="editRoomAdults(3,'+i+')">3</li></ul></div></div><hr style="border: 1px solid #cfd1d2;"></div>';
  ...
  $.ajax({
     ...
     success:function(response){
        if(response.avails_room == 0){
           $('#selected_room_category').text($('#room_category_filter option:selected').val().replace('_', ' '));
           $('.no_room_avail_error_msg_filter').removeClass('d-none').fadeIn().delay(5000).fadeOut()
